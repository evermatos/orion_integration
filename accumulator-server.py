...
from __future__ import division   # need for seconds calculation (could be removed with Python 2.7) # noqa
__author__ = 'fermin'

# This program stores everything it receives by HTTP in a given URL
# (pased as argument),
# Then return the accumulated data upon receiving 'GET <host>:<port>/dump'.
# It is aimet at harness test for subscription scenarios
# (so accumulator-server.py plays the role of a subscribed application)
#
# Known issues:
#
# * Curl users: use -H "Expect:" (default "Expect: 100-continue" used by
# curl has been problematic in the past)
# * Curl users: use -H "Content-Type: application/xml"  for XML payload
# (the default: "Content-Type: application/x-www-form-urlencoded" has
# been problematic in the pass)
...
def usage():
    ...
    print ('Usage: %s --host <host> --port <port> --url <server url>' +
           ' --pretty-print -v -u' % os.path.basename(__file__))
    print ('')
    print ('Parameters:')
    print ("  --host <host>: host to use database to use (default is '0.0.0.0')")  # noqa
...
try:
    opts, args = getopt(sys.argv[1:], 'vu', ['host=', 'port=', 'url=',
                                             'pretty-print', 'https',
                                             'key=', 'cert='])
except GetoptError:
...
if os.path.isfile(pidfile):
    ...
    print ("PID file %s already exists, killing the process %s" % (pidfile, oldpid))  # noqa
...
# This response has been designed to test the #2360 case,
# but is general enough to be
# used in other future cases (e.e. #3363)
@app.route("/badresponse/queryContext", methods=['POST'])
def bad_response():
    r = Response(status=404)
    r.data = '{"name":"ENTITY_NOT_FOUND","message":"The entity with the requested id [qa_name_01] was not found."}'  # noqa
    return r

# From https://stackoverflow.com/questions/14902299/json-loads-allows-duplicate-keys-in-a-dictionary-overwriting-the-first-value # noqa
...
def record_request(request):
    ...
    if (t0 == ''):
        ...
        t = (delta.microseconds + (delta.seconds + delta.days * 24 * 3600) * 10 ** 6) / 10 ** 6  # noqa
        times.append(trunc(round(t)))
        # times.append(t)

    # Store verb and URL
    #
    # We have found that request.url can be problematic in some
    # distributions (e.g. Debian 8.2)
    # when used with IPv6, so we use request.scheme,
    # request.host and request.path to compose
    # the URL "manually"
    #
    #  request.url = request.scheme + '://' + request.host + request.path
    #
    s += request.method + ' ' + request.scheme + '://' + request.host + request.path  # noqa
    ...
    if ((request.data is not None) and (len(request.data) != 0)):
        ...
        if pretty is True:
            try:
                raw = json.loads(request.data, object_pairs_hook=dict_raise_on_duplicates)  # noqa
...
def record_2871():
    ...
    if send_continue(request):
        ...
        # Ad hoc response related with issue #2871, see https://github.com/telefonicaid/fiware-orion/issues/2871 # noqa
        r = Response(status=200)
        r.data = '{"contextResponses":[{"contextElement":{"attributes":[{"name":"turn","type":"string","value":""}],"id":"entity1","isPattern":false,"type":"device"},"statusCode":{"code":200,"reasonPhrase":"OK"}}]}'  # noqa
        return r

# Next 6 ones are for testing subscription status and failure logic.
# They are used by test
...
if __name__ == '__main__':
    # Note that using debug=True breaks the the procedure to write the PID
    # into a file. In particular
    # makes the calle os.path.isfile(pidfile) return True, even if the
    # file doesn't exist. Thus,
    # use debug=True below with care :)
    if (https):
        # According to http://stackoverflow.com/questions/28579142/attributeerror-context-object-has-no-attribute-wrap-socket/28590266, the # noqa
        # original way of using context is deprecated. New way is simpler. However, we are still testing this... some environments # noqa
        # fail in some configurations (the current one is an attempt to make this to work at jenkins) # noqa
